...
function calculateAvailability(chain){
    ...
    */
    ...
    //this variable is used to verify if every states are reached
    let chainReachabilityCount = []

    for(let i = 0; i < chain.length; i++){
        chainReachabilityCount.push(0)
    }

    chain.forEach( (state, index) => {

        if(state.transitions == undefined || state.transitions.length == 0){
            throw new ChainException(`state ${state.number} is an absortion state`, "AbsorbingStateChain")
        }

        state.transitions.forEach( tr => {
            if(tr.to === state.number){
                throw new ChainException(`state ${state.number} has a self transition`, "SelfTransitionChain")
            }

            chainReachabilityCount[tr.to-1]++
        })

    })

    chainReachabilityCount.forEach( (number, index) => {
        if(number === 0){
            throw new ChainException(`state ${index + 1} is unreachable`, "ReachabilityMapBroken")
        }
    })

    return CalculateSteadyStateProbabilitiesArray(chain)
